...
export default defineComponent({
    ...
    props: {
        ...
        useSeconds: Boolean,
        line_ids :Array as PropType<number[]>,
        OFE: String,
    },
    setup(props, context) {
        const {
            overspeedInfo,
            scope,selectedDates,selectedHours,useSeconds,line_ids,OFE
        } = toRefs(props);
        ...
        const id2NameMap = computed(()=>{
            return _(line_ids!.value! as number[])
                .map((v)=>{
                    return [v, buslineInfoGetter(v).line_name]
                })
                .fromPairs()
                .value()
        })
        ...
        const basicLineChartData = computed(()=>{
            return _(filtedInfo.value)
                .groupBy(v=>id2NameMap.value[v.line_id])
                .map((v, name)=>{
                    const res = 
                        _(v)
